import _styled from "@emotion/styled/base";
import { jsx as ___EmotionJSX } from "@emotion/core";
function _EMOTION_STRINGIFIED_CSS_ERROR__() { return "You have tried to stringify object returned from `css` function. It isn't supposed to be used directly (e.g. as value of the `className` prop), but rather handed to emotion so it can handle it (e.g. as value of `css` prop)."; }
import React from 'react';
import { Card } from '../../';
var ShortDiv = /*#__PURE__*/_styled('div', {
  target: "ejro7z50"
})(process.env.NODE_ENV === "production" ? {
  name: "1gkdjtj",
  styles: "height:300px;max-height:300px;& > *{max-height:100%;}"
} : {
  name: "1gkdjtj",
  styles: "height:300px;max-height:300px;& > *{max-height:100%;}",
  map: "/*# sourceMappingURL=data:application/json;charset=utf-8;base64, */",
  toString: _EMOTION_STRINGIFIED_CSS_ERROR__
});
export default {
  title: 'Components/Card',
  component: Card
};
export var defaultStory = function defaultStory() {
  return ___EmotionJSX(Card, null, ___EmotionJSX(Card.Header, null, "Header"), ___EmotionJSX(Card.Body, null, "Body", ___EmotionJSX("br", null), ___EmotionJSX("br", null), ___EmotionJSX("br", null), ___EmotionJSX("br", null)), ___EmotionJSX(Card.Footer, null, "Footer"));
};
defaultStory.story = {
  name: 'default'
};
export var withCustomPaddings = function withCustomPaddings() {
  return ___EmotionJSX(Card, null, ___EmotionJSX(Card.Header, {
    padding: "xl"
  }, "Header"), ___EmotionJSX(Card.Body, {
    padding: "xl"
  }, "Body", ___EmotionJSX("br", null), ___EmotionJSX("br", null), ___EmotionJSX("br", null), ___EmotionJSX("br", null)), ___EmotionJSX(Card.Footer, {
    padding: "xl"
  }, "Footer"));
};
withCustomPaddings.story = {
  name: 'with custom paddings'
};
export var withMultipleSections = function withMultipleSections() {
  return ___EmotionJSX(Card, null, ___EmotionJSX(Card.Header, null, "Header"), ___EmotionJSX(Card.Body, {
    padding: "none"
  }, ___EmotionJSX(Card.Section, null, "Section", ___EmotionJSX("br", null), ___EmotionJSX("br", null), ___EmotionJSX("br", null), ___EmotionJSX("br", null)), ___EmotionJSX(Card.Section, null, "Section", ___EmotionJSX("br", null), ___EmotionJSX("br", null), ___EmotionJSX("br", null), ___EmotionJSX("br", null)), ___EmotionJSX(Card.Section, null, "Section", ___EmotionJSX("br", null), ___EmotionJSX("br", null), ___EmotionJSX("br", null), ___EmotionJSX("br", null))), ___EmotionJSX(Card.Footer, null, "Footer"));
};
withMultipleSections.story = {
  name: 'with multiple sections'
};
export var withScrollableBody = function withScrollableBody() {
  return ___EmotionJSX(ShortDiv, null, ___EmotionJSX(Card, null, ___EmotionJSX(Card.Header, null, "Header"), ___EmotionJSX(Card.Body, {
    padding: "none",
    scrollable: true
  }, ___EmotionJSX(Card.Section, null, "Section", ___EmotionJSX("br", null), ___EmotionJSX("br", null), ___EmotionJSX("br", null), ___EmotionJSX("br", null), ___EmotionJSX("br", null), ___EmotionJSX("br", null), ___EmotionJSX("br", null), ___EmotionJSX("br", null)), ___EmotionJSX(Card.Section, null, "Section", ___EmotionJSX("br", null), ___EmotionJSX("br", null), ___EmotionJSX("br", null), ___EmotionJSX("br", null), ___EmotionJSX("br", null), ___EmotionJSX("br", null), ___EmotionJSX("br", null), ___EmotionJSX("br", null)), ___EmotionJSX(Card.Section, null, "Section", ___EmotionJSX("br", null), ___EmotionJSX("br", null), ___EmotionJSX("br", null), ___EmotionJSX("br", null), ___EmotionJSX("br", null), ___EmotionJSX("br", null), ___EmotionJSX("br", null), ___EmotionJSX("br", null))), ___EmotionJSX(Card.Footer, null, "Footer")));
};
withScrollableBody.story = {
  name: 'with scrollable body'
};
export var withoutFooter = function withoutFooter() {
  return ___EmotionJSX(Card, null, ___EmotionJSX(Card.Header, null, "Header"), ___EmotionJSX(Card.Body, null, "Body", ___EmotionJSX("br", null), ___EmotionJSX("br", null), ___EmotionJSX("br", null), ___EmotionJSX("br", null)));
};
withoutFooter.story = {
  name: 'without footer'
};
export var withoutHeader = function withoutHeader() {
  return ___EmotionJSX(Card, null, ___EmotionJSX(Card.Body, null, "Body", ___EmotionJSX("br", null), ___EmotionJSX("br", null), ___EmotionJSX("br", null), ___EmotionJSX("br", null)), ___EmotionJSX(Card.Footer, null, "Footer"));
};
withoutHeader.story = {
  name: 'without header'
};