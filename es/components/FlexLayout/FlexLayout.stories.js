import _styled from "@emotion/styled-base";
import { jsx as ___EmotionJSX } from "@emotion/core";
import React, { Fragment } from 'react';
import { Row, Column } from '../../';

var ColorBlock =
/*#__PURE__*/
_styled('div', {
  target: "ek5mgdl0"
})(function (props) {
  return {
    display: 'flex',
    alignItems: 'center',
    justifyContent: 'center',
    minWidth: '100px',
    padding: '0 10px',
    minHeight: '30px',
    backgroundColor: props.color,
    border: '1px solid gray'
  };
}, process.env.NODE_ENV === "production" ? "" : "/*# sourceMappingURL=data:application/json;charset=utf-8;base64, */");

var BorderBlock =
/*#__PURE__*/
_styled('div', {
  target: "ek5mgdl1"
})(process.env.NODE_ENV === "production" ? {
  name: "1l68nnx",
  styles: "border:1px solid gray;"
} : {
  name: "1l68nnx",
  styles: "border:1px solid gray;",
  map: "/*# sourceMappingURL=data:application/json;charset=utf-8;base64, */"
});

var Blocks = function Blocks(_ref) {
  var children = _ref.children;
  return ___EmotionJSX(Fragment, null, ___EmotionJSX(ColorBlock, {
    color: "#e2e4e9"
  }, children), ___EmotionJSX(ColorBlock, {
    color: "#e2e4e9"
  }), ___EmotionJSX(ColorBlock, {
    color: "#e2e4e9"
  }), ___EmotionJSX(ColorBlock, {
    color: "#e2e4e9"
  }), ___EmotionJSX(ColorBlock, {
    color: "#e2e4e9"
  }));
};

var Block = function Block(_ref2) {
  var children = _ref2.children;
  return ___EmotionJSX(ColorBlock, {
    color: "#e2e4e9"
  }, children);
};

export default {
  title: 'Components/FlexLayout',
  component: Row
};
export var defaultRow = function defaultRow() {
  return ___EmotionJSX(Row, null, ___EmotionJSX(Blocks, null, "Row"));
};
defaultRow.story = {
  name: 'default Row'
};
export var defaultColumn = function defaultColumn() {
  return ___EmotionJSX(Column, null, ___EmotionJSX(Blocks, null, "Column"));
};
defaultColumn.story = {
  name: 'default Column'
};
export var withJustifyContent = function withJustifyContent() {
  return ___EmotionJSX(Row, {
    justifyContent: "end"
  }, ___EmotionJSX(Blocks, null, "justifyContent=\"end\""));
};
withJustifyContent.story = {
  name: 'with justifyContent'
};
export var withAlignContent = function withAlignContent() {
  return ___EmotionJSX(Column, {
    alignItems: "stretch"
  }, ___EmotionJSX(Blocks, null, "alignItems=\"stretch\""));
};
withAlignContent.story = {
  name: 'with alignContent'
};
export var withGap = function withGap() {
  return ___EmotionJSX(Column, null, ___EmotionJSX(Row, {
    gap: "xs"
  }, ___EmotionJSX(Blocks, null, "xs")), ___EmotionJSX(Row, {
    gap: "sm"
  }, ___EmotionJSX(Blocks, null, "sm")), ___EmotionJSX(Row, {
    gap: "md"
  }, ___EmotionJSX(Blocks, null, "md")), ___EmotionJSX(Row, {
    gap: "lg"
  }, ___EmotionJSX(Blocks, null, "lg")), ___EmotionJSX(Row, {
    gap: "xl"
  }, ___EmotionJSX(Blocks, null, "xl")));
};
withGap.story = {
  name: 'with gap'
};
export var withOffset = function withOffset() {
  return ___EmotionJSX(Row, null, ___EmotionJSX(BorderBlock, null, ___EmotionJSX(Row, {
    offsetY: "none",
    offsetX: "none"
  }, ___EmotionJSX(Block, null, "none"))), ___EmotionJSX(BorderBlock, null, ___EmotionJSX(Row, {
    offsetY: "xs",
    offsetX: "xs"
  }, ___EmotionJSX(Block, null, "xs"))), ___EmotionJSX(BorderBlock, null, ___EmotionJSX(Row, {
    offsetY: "sm",
    offsetX: "sm"
  }, ___EmotionJSX(Block, null, "sm"))), ___EmotionJSX(BorderBlock, null, ___EmotionJSX(Row, {
    offsetY: "md",
    offsetX: "md"
  }, ___EmotionJSX(Block, null, "md"))), ___EmotionJSX(BorderBlock, null, ___EmotionJSX(Row, {
    offsetY: "lg",
    offsetX: "lg"
  }, ___EmotionJSX(Block, null, "lg"))), ___EmotionJSX(BorderBlock, null, ___EmotionJSX(Row, {
    offsetY: "xl",
    offsetX: "xl"
  }, ___EmotionJSX(Block, null, "xl"))));
};
withOffset.story = {
  name: 'with offset'
};
export var withGrowChildren = function withGrowChildren() {
  return ___EmotionJSX(Row, {
    growChildren: true
  }, ___EmotionJSX(Blocks, null, "growChildren"));
};
withGrowChildren.story = {
  name: 'with grow children'
};
export var withPointerCursor = function withPointerCursor() {
  return ___EmotionJSX(Row, {
    cursor: "pointer"
  }, ___EmotionJSX(Blocks, null, "cursor=\"pointer\""));
};
withPointerCursor.story = {
  name: 'with pointer cursor'
};