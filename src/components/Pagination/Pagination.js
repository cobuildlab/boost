...
class Pagination extends React.Component<PaginationProps, PaginationState> {
  ...
  // $FlowFixMe
  getPageSizeOptions = (pageSizeOptions: number[]) =>
    pageSizeOptions.map((value) => ({ value, label: String(value) }));

  render() {
    const {
      page: pageFromProps,
      pageSize: pageSizeFormProps,
      total,
      pageSizeOptions = [],
      showSizeChanger,
    } = this.props;
    ...
    const pages = [...Array(VISIBLE_RANGE * 2 + 1).keys()].slice(
      0,
      numberOfPages
    );
    ...
    return (
      <PaginationTag tagName="div">
        ...
        <PaginationItemsTag>
          ...
          {pages.map((index) => (
            <PaginationItemTag
              ...
              onClick={this.createOnChange(index + start)}
            >
              {index + start}
            </PaginationItemTag>
          ))}
          ...
          <If condition={rightSide < numberOfPages && showRightMore}>
            <PaginationItemTag
        ...
        <Text
          ...
          `}
        >
          {formatter.format(firstRecordIndex)} -{" "}
          {formatter.format(lastRecordIndex)} of {formatter.format(total)}{" "}
          records
        </Text>
      </PaginationTag>
    );
